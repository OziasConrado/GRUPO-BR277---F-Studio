...
const indicatePointSchema = z.object({
  ...
  imageFile: z.custom<File | undefined>()
    .refine(
      (file) => file instanceof File, { message: "Foto do local é obrigatória." }
    ).refine(
...
export type IndicatePointSubmitData = Omit<IndicatePointFormValues, 'imageFile'> & {
    imageFile: File;
};
...
export default function IndicatePointModal({ isOpen, onClose, onSubmit, isSubmitting }: IndicatePointModalProps) {
  ...
  const handleFormSubmit = (data: IndicatePointFormValues) => {
    // We can safely assert imageFile is a File because the schema validation will have caught it if it's not.
    onSubmit(data as IndicatePointSubmitData);
  };
