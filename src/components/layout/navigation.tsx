...
import { Home, Wrench, Video, AlertTriangle, MessageSquare } from 'lucide-react'; // Changed Headset to MessageSquare
...
const navItems = [
  ...
  { href: '/chat', label: 'Chat', icon: MessageSquare }, // Changed from SAU to Chat
];

export default function Navigation() {
  const pathname = usePathname();
  const { openChat, setIsChatOpen } = useChat(); // Get openChat from context

  const handleNavItemClick = (e: React.MouseEvent, href: string) => {
    if (href === '/chat') {
      e.preventDefault(); // Prevent navigation for the chat item
      setIsChatOpen(true);   // Open the chat modal
    }
    // Allow default navigation for other items
  };
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-40 h-[65px] border-t bg-background/95 backdrop-blur-md shadow-[0_-2px_10px_rgba(0,0,0,0.05)] sm:hidden">
        <div className="container mx-auto grid h-full grid-cols-5 items-stretch px-1">
          {navItems.map((item, index) => {
            const isActive = pathname === item.href && item.href !== '/chat'; // Chat is not a page, so active state doesn't apply same way
              ...
              <>
                <item.icon
                  className={cn(
                    ...
                    isChatButton && 'text-primary' // Example: Keep chat icon primary or adjust as needed
                  )}
                />
                <span className={cn(
                  ...
                  !isCentralButton && !isActive && 'group-hover:text-primary/80',
                   isChatButton ? 'text-primary font-semibold' : '' // Example: Keep chat text primary
                )}>
                  {item.label}
                </span>
              </>
            ...
            const commonClasses = cn(
              'group relative flex h-full flex-col items-center justify-center text-center no-underline transition-colors duration-150 pt-1 pb-0.5',
               ...
               isChatButton ? 'text-primary font-semibold' : '', // Ensure chat button has distinct styling if needed
